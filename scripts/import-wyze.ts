...
import type { WyzeProduct } from '../src/lib/scrapers/wyze-scraper';
...
function toRow(p: WyzeProduct): ProductRow {
  return {
...
const DATA_PATH = path.join(__dirname, '..', 'data', 'wyze-products.json');
const BATCH_SIZE = 50;
...
export async function importWyze(): Promise<ImportResult> {
  console.log('='.repeat(60));
  console.log('Wyze Import — Supabase');
  console.log('='.repeat(60));

  if (!fs.existsSync(DATA_PATH)) {
    console.error(`\nData file not found: ${DATA_PATH}`);
    console.error('Run the scraper first with: npx ts-node scripts/run-wyze.ts');
    process.exit(1);
  }

  const raw = fs.readFileSync(DATA_PATH, 'utf-8');
  const products: WyzeProduct[] = JSON.parse(raw);
  ...
  if (fetchErr) {
    ...
    // Continue — duplicate errors will surface per row
  } else if (existing) {
    for (const row of existing) {
      existingIds.add(row.product_id);
    }
    console.log(`  ${existingIds.size} Wyze products already in database`);
  }

  // ── Step 2: Separate new vs existing ────────────────────────────────────
  const newProducts: WyzeProduct[] = [];
  const skippedProducts: WyzeProduct[] = [];
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    if (insertErr) {
      ...
      // Fall back to one-by-one so partial failures don't block the whole batch
