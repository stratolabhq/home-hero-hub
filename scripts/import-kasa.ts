...
import type { KasaProduct } from '../src/lib/scrapers/kasa-scraper';
...
function toRow(p: KasaProduct): ProductRow {
  return {
...
const DATA_PATH = path.join(__dirname, '..', 'data', 'kasa-products.json');
const BATCH_SIZE = 50;
...
export async function importKasa(): Promise<ImportResult> {
  console.log('='.repeat(60));
  console.log('TP-Link Kasa Import — Supabase');
  console.log('='.repeat(60));

  if (!fs.existsSync(DATA_PATH)) {
    console.error(`\nData file not found: ${DATA_PATH}`);
    console.error('Run the scraper first with: npx ts-node scripts/run-kasa.ts');
    process.exit(1);
  }

  const raw = fs.readFileSync(DATA_PATH, 'utf-8');
  const products: KasaProduct[] = JSON.parse(raw);
  ...
  const { data: existing, error: fetchErr } = await supabase
    .from('products')
    .select('product_id')
    .eq('brand', 'TP-Link Kasa');

  if (fetchErr) {
    console.error('Failed to fetch existing products:', fetchErr.message);
  } else if (existing) {
    for (const row of existing) existingIds.add(row.product_id);
    console.log(`  ${existingIds.size} TP-Link Kasa products already in database`);
  }

  // ── Step 2: Separate new vs existing ─────────────────────────────────────
  const newProducts: KasaProduct[] = [];
  const skippedProducts: KasaProduct[] = [];
  ...
  if (skippedProducts.length > 0) {
    ...
    for (const p of skippedProducts) console.log(`  - ${p.name}`);
  }
